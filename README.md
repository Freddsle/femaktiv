...
Anymize receives the submitted message, current chat context and attached copies and owns masking before the downstream model and any response restoration. The app uses Anymize's combined endpoint and consumes its returned LLM answer directly; each of the two conversational model steps uses this same endpoint. femaktiv does not detect, redact or verify personal identifiers in normal chat. Provider placeholders and returned identifiers are accepted, so displayed replies are not guaranteed to be masked. The app still requires `_anymize.anonymized: true` and validates completion, response size, schema and citations; it rejects empty displayed prose and raw generated URLs. The optional developer masking inspection below is separate from normal chat and is not a setup prerequisite.

The intended provider masking coverage includes names, street addresses, exact birth dates, emails/phones, financial and identity numbers, and legal case/contract identifiers. The app does not request identity mappings or reconstruct identifiers. Provider metadata and the displayed conversation do not establish exactly what the downstream model received; the assistant must not claim it verified masking or account settings. Health and legal details remain sensitive. Anymize is the only runtime external service. No credentials or private payloads are written to application logs. Account ZDR and disabled provider fallbacks remain required; response restoration depends on Anymize's behavior under those settings.
