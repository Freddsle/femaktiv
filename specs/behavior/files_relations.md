...
| [Live chat](live_chat.md) | Additional ownership of `accounts/`, `chats/`, their unit tests and templates, `content/evidence.json`, live-chat and privacy browser tests, chat JavaScript, shared site CSS for live controls, AI settings and the local launcher. |
...
The privacy browser suite `tests/e2e/test_privacy.py` covers registration/access denial, the account-settings deletion action and confirmation templates, retained quota records, and chat draft handling. Its explicit paired mappings MUST include the corresponding account views/forms/models, chat admission/accounting, JavaScript and templates.
