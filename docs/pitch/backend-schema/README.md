...
“femaktiv draws on two distinct inputs: her private context and a curated library of public guidance. Her questions, selected notes and active conversation belong to her; the source library contains selected published guidance. The Django backend selects relevant evidence and guides each model request. Anymize handles identifier masking before the OpenAI model processes it. femaktiv checks returned references and presents structured guidance, in English or German. When she asks, it can save information to My notes. When an urgent referral is flagged, it adds German help numbers from fixed application records.”

The slide is a conceptual view of the configured live prototype, which requires operator-approved tester access and server-side provider configuration. Placeholder mode remains the offline default. Live turns begin with structured intake through Anymize and use at most two model calls within a shared 60-second deadline:

| Intake outcome | Remaining work | Model calls for a successful turn |
| --- | --- | --- |
| Clarification needed | Return focused questions; include the fixed urgent-help panel if a referral is flagged. | 1 |
| Standalone request to save a note | Validate the draft and persist the note with the completed turn; return an application-owned receipt and note link. | 1 |
| Immediate danger | Return the application-owned urgent response and German help panel; persist a requested note if one was drafted. | 1 |
| Answer, optionally combined with a save request | Select local evidence, compose through Anymize, validate the response and supplied reference IDs, then persist messages, citation snapshots and any requested note or urgent-help panel. Useful follow-up questions can accompany the answer. | 2 |

The combined AI card represents intake and conditional composition. Requested note saving and the fixed help panel add no model calls. The model badge identifies **GPT-5.6**, matching the operator's configured `openai/gpt-5.6-sol` checked on 13 September 2026. That configuration check belongs to the original artwork; this documentation refresh did not recheck model access or invoke a provider. The label is explicit presentation copy in `MODEL_LABEL`; the renderer never reads credentials or local environment files. Update it when the configured model changes. The diagram does not assert that model selection automatically tracks the newest ChatGPT release, and ChatGPT's consumer application is not part of this API integration.

Personal notes and chat history are not source-library records. The separate input arrows describe distinct ownership and purposes; relevant context and source passages are brought together for answer composition. Selected notes are pinned as immutable copies until explicitly removed or refreshed. Removing or refreshing a copy, or resetting conversation history, starts a new segment; a history reset retains pinned copies. Saving a new note from a chat requires an explicit request in the latest user message and does not automatically pin it. The backend creates it for the chat owner in the same guarded transaction as the completed turn; duplicate submissions reuse the saved result. Keeping conversation context alone is not a save to My notes.

The German help panel comes from fixed, source-checked application records when intake or composition flags an urgent or emergency referral. Its saved wording says “If you are in Germany”; language does not establish location. It is separate from retrieved citations and involves no runtime search or additional provider call. This documentation refresh describes existing behavior; it does not change storage, provider routing or website wording.
...
- Architecture: [live-chat specification](../../../specs/behavior/live_chat.md), [service](../../../chats/service.py), [turn persistence](../../../chats/turns.py), [active context](../../../chats/context.py), [urgent-help records](../../../chats/urgent_help.py), [provider](../../../chats/provider.py) and [evidence library](../../../content/evidence.json).
...
The refreshed PNG was visually reviewed at full-slide scale, including the expanded output panel, separate context/source panels and nested OpenAI badge. All SVG text bounds fit the canvas. Export checks confirm a 3840 × 2160 PNG and a single 16:9 PDF page with selectable text. Diagram content was reviewed against the current local source library, conditional one-/two-call routing, requested note persistence and urgent-help records. Repository relation/link validation, Donna artifact validation and whitespace checks were run separately from application tests; this artwork does not change application behavior. No application regression tests or live provider checks were run for this documentation refresh.
