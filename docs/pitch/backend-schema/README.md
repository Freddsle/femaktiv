...
A 16:9 slide explaining the prototype through its value to the user: personal context, credible sources and practical support. The composition uses the existing website palette, a prominent femaktiv coordination layer, the approved femaktiv logo and Anymize's official wordmark.
...
“femaktiv starts with her question and the context she chooses to share. It selects relevant passages from a curated evidence library and guides the model with clear instructions. Both understanding the question and drafting an answer go through Anymize, which handles identifier masking before the model. femaktiv checks the returned source references, then presents practical guidance with links she can explore, in English or German.”
...
The PNG was visually reviewed at full-slide scale, and all SVG text bounds fit the canvas. Export checks confirm a 3840 × 2160 PNG and a single 16:9 PDF page with selectable text. Diagram content was reviewed against the current local source library and two-call provider flow. Repository relation/link validation, Donna artifact validation and whitespace checks were run separately from application tests; this artwork does not change application behavior.
