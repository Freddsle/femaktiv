...
The operator’s 19:41:06 UTC evaluation again passed English nutrition; German intake completed, but composition failed with `deadline_exceeded` / `timeout`. JSON provider requests now use the remaining shared 60-second turn budget instead of a separate 25-second per-request cap. The total deadline, browser timeout and server timeout are unchanged. All 31 focused transport/provider/evaluation tests passed, including allowance reduction and rejection after expiry. This removes an avoidable local timeout limit; success for the full bilingual live evaluation remains unverified. The private prototype can be used with the observed partial results, and the operator may defer the full paid evaluation. Restart the website to load the fix; ngrok needs no change.

Timeout follow-up verification passed: 153 backend tests, 15 browser tests, lint/format, Django checks, translations/static collection, Depmesh, Donna validation and whitespace checks. The initial browser run was blocked by sandbox socket permissions; the permitted local rerun passed. The existing bilingual and isolated production review remains applicable because the change only removes the shorter provider-call timeout. No additional paid provider evaluation was run by the agents.

### Provider-owned masking and response restoration — 13 September 2026

The user clarified that Anymize owns masking and any restoration, and that femaktiv should not verify personal identifiers itself. Normal-chat output checks now accept provider placeholders and returned identifiers, including names, addresses, email/phone details and identity numbers. Schema, completion, response bounds, supplied source IDs, empty-prose and raw generated URL checks remain. The application still requires `_anymize.anonymized: true`, account ZDR confirmation and disabled provider fallbacks; it does not request mappings or reconstruct identifiers. Prompts leave identifier handling to Anymize and discourage unsupported claims that the assistant inspected masking or account settings.

Displayed replies are not guaranteed to remain masked. The [Anymize API documentation](https://app.anymize.ai/api-docs/anonymization) describes ordinary response restoration and says de-anonymization is unavailable under account ZDR; provider account settings and actual masking/restoration behavior were not changed or verified here. The conversation and metadata alone do not establish what the downstream model received. The existing developer export evaluator remains optional and unused for this change; it is not part of runtime or a setup prerequisite.

All 45 focused provider/service/turn/evaluation tests passed with mocked services. The revised regression accepts fictional returned identifiers in English and provider placeholders in German through the actual adapter with positive anonymization metadata. Final Donna verification passed: **154 backend tests and 15 browser tests**, 309 translations, Ruff lint/format, Django system/migration checks, static collection, Depmesh, workflow validation and whitespace checks. Earlier checks of the removed local identifier guard do not validate this implementation. No provider credits were consumed.

An isolated production check also passed in English/German at 390px/1440px with DEBUG=False Gunicorn/WhiteNoise, a temporary SQLite database/key and mocked Anymize transport. Returned fictional identifiers and placeholders were accepted, rendered, saved and reloaded unchanged. Four turns made exactly eight mocked model calls; duplicate POSTs, status recovery and elapsed browser time made none extra. Screenshots were reviewed without overflow or JavaScript errors, and temporary resources were removed. Evidence is ignored `.session/donna/live-chat/provider-owned-production-report.json` and adjacent screenshots. Restart the application to load the change; no ZDR or ngrok setting change is needed.


## Chat-list deletion — 13 September 2026

Added a visible trash action beside each saved chat in the desktop and mobile history list, reusing the existing English/German labels, confirmation page and owner-scoped deletion endpoint. Chat-title updates and truncation remain intact; keyboard focus stays visible inside the scrollable list. This change adds only template/CSS behavior and does not change the deletion backend.

Verified 16 focused chat-view tests, all 158 backend tests and all 15 existing browser tests. The initial full check encountered two failing cases in an independently edited live-chat repair; the backend rerun passed after those changes landed. Translation compilation (309 strings), Ruff, Django checks, missing-migration checks, static collection, Depmesh, Donna validation and whitespace checks passed. A separate isolated Gunicorn/WhiteNoise browser check passed in English/German at 390px/1440px: visible delete actions, long-title layout, keyboard activation, cancellation, non-current/current/final-chat deletion and the empty list. Screenshots are ignored `.local/screenshots/chat-delete-{en,de}-{390,1440}.png`. Temporary fictional data and the local test server were removed; no provider calls were made. The independently progressing Donna live-chat repair session was preserved.

## Chat submission waiting indicator — 13 September 2026

The existing localized pending status now appears in a prominent lavender panel with a spinner, and the disabled Send button shows a spinner throughout the request. The change uses the composer's existing busy state and shared CSS; approved wording, submission handling and draft recovery remain intact. The existing reduced-motion preference disables both animations.

Donna's full offline check passed: 154 backend tests, 15 browser tests, 309 translations, Ruff lint/format, Django system and missing-migration checks, static collection, Depmesh, workflow validation and whitespace checks. The initial browser run could not open its local test socket in the sandbox; the permitted rerun passed. An isolated DEBUG=False Gunicorn/WhiteNoise browser review also passed in English/German at 390px/1440px with held responses: the status stayed within the viewport, both spinners appeared, reduced motion stopped them, successful replies cleared the pending state, and network failures restored controls while keeping an editable draft. No horizontal overflow or JavaScript errors occurred. Evidence and reviewed screenshots are ignored `.session/donna/live-chat/waiting-production-report.json` and `waiting-{en,de}-{390,1440}.png`. Temporary server/database resources were removed; no provider calls were made. Restart an already-running production preview and refresh the page to load the newly collected stylesheet.

## Simple German urgent-help panel — 13 September 2026

Urgent intake now adds a small, application-owned panel with 112 and 116117. A structured referral flag in the existing intake/composition calls also covers replies recommending prompt medical assessment; either step can request the panel. There is no additional classifier, provider, background worker, search or geolocation. The heading is conditional (“If you are in Germany” / “Wenn du in Deutschland bist”), and each number has its purpose, an official source link and a telephone link. Existing urgent reply wording and public examples remain unchanged. Saved panel snapshots keep their original language and survive duplicate/status recovery and reload; existing messages are not backfilled.

National service boundaries were checked against the [federal health portal](https://gesund.bund.de/notfallnummern) and [116117's English guidance](https://www.116117.de/de/englisch.php): 112 for danger to life or possible lasting harm; 116117 for urgent non-life-threatening problems outside practice hours that cannot wait until reopening. The panel notes that 116117 telephone advice is in German. These sources verify the contact information, not the model's medical classification accuracy.

Donna's final offline gate passed: **162 backend tests and 16 browser tests**, 314 German translations, Ruff lint/format, Django system/missing-migration checks, static collection, Depmesh, workflow validation and whitespace checks. The first attempt found browser-test formatting, which was fixed; the next attempt was unable to open local test sockets in the sandbox, and the permitted rerun passed. The 42 focused service/provider/turn/panel tests also passed. Routine tests used mocks and the offline guard; no provider credits were consumed.

An isolated DEBUG=False Gunicorn/WhiteNoise check passed 12 fictional turns with 19 mocked model calls, covering urgent intake, referrals in composition/clarification, retained intake flags and ordinary replies. English/German at 390px/1440px passed dynamic/reloaded rendering, saved-language preservation, keyboard telephone links, official source links and no horizontal overflow or JavaScript errors. Duplicate submissions, status recovery, reload and elapsed browser time made no additional calls. Four screenshots were visually reviewed. The temporary server/database were removed; evidence is ignored `.session/donna/live-chat/urgent-help-production-report.json` and adjacent `urgent-help-production-{en,de}-{390,1440}.png` screenshots.

Migration `chats.0004_message_urgent_help` was applied to the existing repository-local SQLite prototype database after checking its engine and path; translations and static assets were rebuilt. Restart the website and refresh the page to load the change. Other installations need `.venv/bin/python manage.py migrate` (or the usual `./bin/setup`) before restarting. Running preview/ngrok processes and provider settings were not changed. Live-model classification with the new flag remains untested; offline routing checks are not clinical validation.

## Structured dialogue with selective follow-up — 13 September 2026

The user asked whether live replies were artificially simplified and could become more useful without losing existing behavior. Inspection found that clarification returned only the intake introduction (at most 600 characters) plus questions, before evidence retrieval. An answer decision containing any intake questions was also rejected, although the final-answer format already supports multiple paragraphs and questions. The supplied conversation is consistent with clarification routing, but its actual provider decisions were not inspected.

Live intake now allows an answer with optional follow-up questions and reserves clarification for information that blocks useful, safe guidance. Composition guidance asks for detail suited to the request: ordered actions, explanation of relevant options supported by the supplied evidence, call preparation, and focused questions. Broad openings and narrow follow-ups may remain brief. Existing urgent routing, referral flags, evidence/citation validation, provider configuration, schema, context and saved messages are preserved. No public example or interface wording was changed. The evidence library still cannot establish current Hamburg contacts, availability or individual service eligibility.

The two-call/60-second budget is unchanged. Some requests that previously ended after intake may now use the second call for a sourced plan; they may therefore take longer and use more provider tokens. No provider calls were made for this change. Mocked tests establish application routing and rendering, not whether the live model will choose the intended path or produce a better answer. Restart the website to load the prompt/routing change; no migration or provider-setting change is needed.

All 27 focused service/evaluation tests passed. Regressions cover bilingual two-turn care conversations, full history and evidence delivery, optional questions in composition, the three-question limit, empty clarification rejection, and retained urgent/referral behavior. An independent code review found no blocking regression.

Donna's full offline gate passed: **166 backend tests and 16 browser tests**, 314 German translations, Ruff lint/format, Django system and missing-migration checks, static collection, Depmesh, workflow validation and whitespace checks. The first browser attempt could not open its local server socket in the sandbox; the permitted rerun passed. No application failure required repair.

An isolated DEBUG=False Gunicorn/WhiteNoise browser review passed 12 fictional turns with 18 mocked model calls. English/German at 390px/1440px exercised clarification followed by six-paragraph cited care plans with an optional question (1,593/1,702 characters), plus concise general and urgent replies. Duplicate submissions, status recovery, reload and 76 simulated browser seconds made no additional calls. Saved paragraphs and citations persisted, drafts cleared, leases completed, and no horizontal overflow, JavaScript errors or external browser requests occurred. Twelve beginning/middle/end screenshots were visually reviewed. The temporary server/database were removed; running previews were untouched. Evidence and the harness are ignored `.session/donna/live-chat/dialog-production-report.json` and `dialog-production-smoke.py`.

## Saving chat information to My notes — 13 September 2026

The reported save request previously remained conversation context because no chat-to-note write path existed. Live intake now supports a bounded create-only note action. A standalone `save_note` decision avoids unnecessary answer composition; combined save-and-advice requests retain ordinary composition and urgent/referral handling. Draft instructions preserve the person concerned and reported uncertainty, without turning unanswered questions or prior AI guidance into personal facts. Existing public examples and approved wording are unchanged.

The completed turn atomically creates an owner-scoped PersonalNote and links it to the assistant message. Only the persistence layer adds the existing localized save confirmation. Immediate and reloaded replies link to the editable note, and the attachment picker gains the new note unchecked. Saving does not pin context. Duplicate/status recovery does not create another note; note deletion clears the link without recreation, while chat deletion retains separately saved notes. Failed, expired, reset or deleted turns cannot leave a saved note behind. Migration `chats.0005_message_saved_note` was applied to the existing repository-local SQLite database after checking its engine and exact path; no personal rows were read or backfilled.

Focused verification passed: 41 service/provider/evaluation tests; 25 turn tests plus the added completed-chat deletion regression; and one browser test exercising all four English/German × 390px/1440px combinations. Initial browser attempts required local socket permission, collection of the changed static asset, and corrections to test setup/absolute-URL expectations; the final focused run passed. An independent review identified the mixed-request shortcut issue, which was corrected before verification.

An isolated DEBUG=False Gunicorn/WhiteNoise smoke passed English/German at both widths with a temporary migrated SQLite database and mocked Anymize transport. Eight turns made exactly eight mocked intake calls. It verified save confirmation/link, editable mother-attributed notes in My notes, immediate/reloaded picker visibility, no automatic pinning, duplicate/status recovery and deletion without recreation. No JavaScript errors, external browser requests, horizontal overflow or extra calls after reload/76 simulated seconds occurred. Chat and notes screenshots were reviewed, and temporary server/database resources were removed. Evidence is ignored `.session/donna/live-chat/save-notes-production-report.json` and adjacent screenshots/harness.

No provider credits were consumed. Real-model interpretation of explicit, negated or quoted save requests and the factual fidelity of generated notes remain untested; mocked checks establish routing and persistence. Restart the website and refresh the page to load the changed schema, prompts and JavaScript, then repeat the save request in the original chat. Existing failed-to-save conversations are not automatically backfilled. Other installations need the usual database migration before restart. Provider configuration and existing preview/tunnel processes were not changed.

Donna's full offline gate passed: **177 backend tests and 17 browser tests**, 314 German translations, Ruff lint/format, Django system/missing-migration checks, static collection, Depmesh (166 files/578 directed edges), workflow validation and whitespace checks. The local browser sockets were enabled for this bounded mocked run. Donna review completed after the production smoke and documentation; live-provider evaluation remains separately unperformed.

## Live chat notice wording — 13 September 2026

At the user's request, the English/German chat notice now identifies the working AI prototype instead of asking users to enter fictional information. It explains that femaktiv sends messages and active context to Anymize for identifier masking before model processing, and retains the disclosure that health information remains sensitive. The wording describes the processing purpose without guaranteeing masking completeness. The existing browser assertion follows the new notice; data handling and the fictional, offline verification policy are unchanged.

Isolated DEBUG=False Gunicorn/WhiteNoise review passed in both languages at 390px/1440px. Exact translated text was visible in all four screenshots without clipping, overlap or horizontal overflow. The check used a temporary database, static files and translation catalogue; no messages, provider attempts, external browser requests or JavaScript errors occurred. Temporary resources were removed. Evidence is recorded in ignored `.session/donna/live-chat/notice-production-report.json` and adjacent screenshots.

Donna's full offline gate passed: **177 backend tests and 17 browser tests**, 314 compiled German translations, Ruff lint/format, Django system/migration checks, static collection, Depmesh, workflow validation and whitespace checks. The initial formatting failure was corrected; the browser run passed after granting local socket access. No provider credits were consumed, and provider masking was not independently evaluated.

## Two real AI pitch examples — 13 September 2026

Implemented the user's approved English pitch preparation in the existing `demo@femaktiv.test` account: two new private chats, the complete fictional report in My notes, two screenshot exports and the [presenter handoff](pitch/use-cases/README.md). No product source, public website wording, account credentials, database schema or provider configuration changed. The existing ngrok/Gunicorn preview was reused with live mode, DEBUG=False, exact HTTPS host/CSRF origin and ngrok inspection disabled.

The report note contains all 8,199 characters of the PDF text after page-layout normalization and correction of three detached superscripts to preserve the printed `10^12/L` and `10^9/L` units. Independent comparison checked the complete note against both rendered PDF pages and confirmed that its active copy and user-message snapshot match. The report belongs only to the food chat; both care turns have no note context.

The explicitly authorized preparation used the real Anymize production turn flow for three sequential user turns and six model calls, with durable request IDs. Lunch completed in 17.16 seconds with a 69-word answer. Initial care completed in 11.94 seconds with an 88-word answer; its insurer wording warranted the plan's single permitted clarification. The refined care answer completed in 13.38 seconds with 88 words and correctly asks the Krankenkasse about conditional temporary support without a care grade. All generated replies remain unchanged in their histories. There were no provider retries. These observations are separate from routine automated checks and do not establish masking completeness or clinical validation.

The selected answers were checked against the existing source passages and six reachable official source pages. They preserve the distinction between the user's fictional report and the mother's care needs, avoid supplement prescriptions and promises of funded services, and display paragraph-level citations. The ten-minute meal preparation estimate is a model suggestion, not a tested recipe.

Authenticated public-browser verification passed for saved-chat reopening, honest live labels, complete note text in My notes, selected context, citation links and completed-turn GET recovery. Private responses use no-store; an existing staff account receives 404 for both chats and the note. English mobile at 390px and German desktop at 1440px preserve saved English messages without horizontal overflow. No JavaScript errors, browser chat mutations or additional provider requests occurred. Temporary inspection sessions were deleted, and hashes confirmed preservation of pre-existing chats, notes, messages and account credentials.

The first browser attempt timed out waiting for network idle while the tunnel streamed the large logo. Subsequent navigation used DOM readiness and focused checks; final screenshot capture explicitly waits for image decoding and fonts. The screenshots crop actual browser content and retain the selected replies and inline citations. The care export shows the final clarification and answer; its original answer remains online.

Django system checks passed and missing-migration checks reported no changes, with the offline guard enabled. Depmesh verification, Donna validation of all workflows and `git diff --check` passed; the scoped Donna workflow finished with no pending work. Both final screenshots were visually reviewed after complete image/font loading. This data/documentation preparation did not rerun or claim the full application regression suite. Reproducible scripts, durable IDs, exact fictional outputs, source review and browser evidence are retained under ignored `.session/donna/pitch-cases/`. Online access lasts while the existing computer, app and tunnel remain running.

## Mobile chat title and notice scrolling — 13 September 2026

At the user's request, the dialog title and AI/mode notice now scroll away with the conversation at widths up to 700px and return at the top. The composer stays outside the mobile scroll area; active context remains accessible. Desktop keeps its original layout and message scroller. Automatic scrolling after loading or sending a message uses the appropriate scroller. All English/German wording is unchanged. Existing browser screenshot helpers now target the mobile scroll area when applicable.

An isolated DEBUG=False Gunicorn/WhiteNoise review passed 200 assertions across English/German, 390/700/701/1440px, and empty/short/long conversations. It checked scroll-away/return behavior, the fixed composer, context controls, breakpoint resizing and browser-mocked successful-send scrolling. Desktop geometry matched the original unwrapped structure exactly. Mobile and desktop screenshots were visually reviewed. Evidence is retained in ignored `.session/donna/live-chat/mobile-scroll-production-report.json` and adjacent screenshots. The temporary server/database were removed; existing preview processes and account data were not changed.

The full offline gate passed 177 backend tests and 17 browser tests, translation compilation, Ruff lint/format, Django system/migration checks, static collection, Depmesh, Donna validation and whitespace checks. The first browser attempt was blocked by sandbox socket permissions; the rerun passed with local test-server access. No provider credits were consumed. Restart the website and refresh the browser to load the updated template and static assets; the existing README startup/tunnel instructions remain applicable.

After adapting the screenshot helpers, all 10 live-chat browser tests passed again; the changed test file passed Ruff lint/format and the final whitespace check passed.
